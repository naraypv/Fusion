...
This system has a fusion agent system installed and functioning. Purge that fully before you start. We will install the modified system after evolving it to production readiness in this machine. The existing daemon key must be preserved from local-only Fusion state for future install of the modified system; do not write the raw key into tracked files.
